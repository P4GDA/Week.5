...
This assignment covers the content in week 5 lectures and tutorials.
...
Dsg2: The DSG2 gene in humans is associated with dilated cardiomyopathy. We will examine some of the phenotypic data for Dsg2 knock out mice.

Hmcn1: The HMCN1 gene in humans is associated with age related macular degeneration. We will examine some of the phenotypic data for Hmcn1 knock out mice.
...
- `sample_type`: indicates if mouse is "mutant" (knock out mouse) or "control"
- `weight`: mouse weight (g)

The Dsg2 data set also contains phenotypic data columns:
- `coat appearance`: indicates if the appearance of the mouse's coat (fur) is "normal" or "abnormal"
...
#### a. 
(1 mark)
Read in the phenotyping data for the Dsg2 gene knock out mice and controls. The data is stored in the CSV: "Dsg2_phenotypic_data.csv" which is located in the directory: "/home/jovyan/Week.5/Week.5.Assignment". Save the data to a variable called "Dsg2_phenotypes".
```{r Assignment.W5.1.a}
# YOUR CODE HERE
```
...
#### c. 
(2 marks)
Create a data frame called "Dsg2_female_CO" that contains only the rows with female mice and only the columns that start with "sample", and the column `cardiac_output`.
```{r Assignment.W5.1.c}
# YOUR CODE HERE
```
...
#### a.
(2 marks)
Use a t-test to determine if the cardiac output is significantly different between control mice and mice with Dsg2 knocked out. Save the p-value of the t-test to a variable called "Dsg2_CO_t.test_p.value".
```{r Assignment.W5.2.a}
# YOUR CODE HERE
```
...
#### b.
(2 marks)
Use a t-test to determine if the left ventricle diameter is significantly different between control mice and mice with Dsg2 knocked out. Save the p-value of the t.test to a variable called "Dsg2_LVD_t.test_p.value".
```{r Assignment.W5.2.b}
# YOUR CODE HERE
```
...
#### a. 
(1 mark)
Read in the phenotyping data for the Hmcn1 gene knock out mice and controls. The data is stored in the CSV: "Hmcn1_phenotypic_data.csv" which is located in the directory: "/home/jovyan/Week.5/Week.5.Assignment". Save the data to a variable called "Hmcn1_phenotypes".
```{r Assignment.W5.3.a}
# YOUR CODE HERE
```
...
#### d. 
(4 marks)
Based on the results from Question 3.b., sexual dimorphism is also present in at least one phenotype for Hmcn1. By calculating the percentage of mutant female and mutant male mice that have abnormal optic disk morphology and by calculating the percentage of mutant female and mutant male mice with a cataract ("present"), determine which phenotype is sexually dimorphic. 

For the sexually dimorphic phenotype, create a data frame called "dimorphic_stats" that includes the following columns:
`sample_type`, `sex`, (the column for the sexually dimorphic phenotype), a column called `mean_weight`, and a column called `min_dark_side_percent`.
The data frame should have one row for each combination of `sample_type`, `sex`, and `cataract`.
```{r Assignment.W5.3.d}
# YOUR CODE HERE
```
