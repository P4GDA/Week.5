...
This tree structure has 4 levels, starting at the top level which has one directory. This is called the *root directory* and it is the directory that contains all other files and directories. All hierarchical directory structures have a root directory, and the root directory does not have a name.
...
Conversely, the root directory is the *parent directory* of `dir_A` and `dir_B`. A parent directory is the directory in which a file or another directory is contained. Files cannot have child directories (because they can't contain a directory), but they do have parent directories.
...
In the above example, `dir1` must be a child directory of the `root directory`, `dir2` must be a child directory of `dir1`, `dir3` must be a child directory of `dir2`, and the final directory or file X must be a child directory of `dir3`.

Because the `root directory` has no name, the first slash can be seen as following the nameless `root directory`.

Let's define some paths for the files in this tree structure.
...
Here is an example of the CSV file format, holding the Ap4e1 phenotypic data for four mutant and four control mice from https://www.mousephenotype.org. This is the same data used in the 5.1 supplement.
...
The first row is the header row which contains the names of each column (this is less commonly present in a TSV file). It is easier to see the five columns (sex, mutant, HG, weight, and grip) in this format compared to the CSV format.
...
```{r}
Ap4e1_data <- read.csv("/home/jovyan/Week.5/5.2.Data.Files/Ap4e1_phenotypes.csv", header=TRUE, stringsAsFactors=FALSE)
Ap4e1_data
```
...
If we want to work with this updated data again without having to recreate this column every time we read the original file into R, we can write this data frame to a new file.
...
Generally you only need to provide the arguments `data_frame`, `file`, and `sep`. However, if you plan to open the file with spreadsheet software, it is recommended to set `row.names` to `FALSE`.

Let's write our updated data frame to a file in the PWD called "Ap4e1_phenotypes_v2.csv":
...
Let's write another file using more of the arguments to see the difference:

```{r}
write.table(Ap4e1_data, "Ap4e1_phenotypes_v3.csv", sep=",", row.names = FALSE, quote=FALSE)
```
